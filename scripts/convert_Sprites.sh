...
input_dir="${data_og_path}/Sprites"
output_dir="${data_out_path}/Sprites"
...
do
    ...
    do
        ...
        ext="${cur_file_out##*.}"
        echo "    "$cur_file_out $ext

        if [ $ext == "gif" ]; then
            ./tools/gif_to_indices $cur_file | oxipng -q -o max --strip all --stdout - | ./tools/gif2gpng $cur_file > $cur_file_out &
            parallel_proc=$((parallel_proc + 1))
        else
            cp "$cur_file" "$cur_file_out"
        fi
        
        # Parallel processing 
        if [ $parallel_proc -eq 2 ]; then
            wait
            parallel_proc=0
        fi
