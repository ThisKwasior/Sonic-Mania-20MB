...
int main(int argc, char** argv)
{
    ...
    if(mdl.magic == RSDK_MDL_MAGIC)
    {
        ...
        for(uint16_t i = 0; i != fvmax; ++i)
        {
            ...
            //                                            pos[1]/scale_facs[1],
            //                                            pos[2]/scale_facs[2]);
            PACK_VEC3_NRM_S16 pos_fp = f32_to_nrm_vec16(pos[0]/scale_facs[0],
                                                        pos[1]/scale_facs[1],
                                                        pos[2]/scale_facs[2]);
            
            //fwrite(&pos_fp, sizeof(PACK_VEC3_NRM_S24), 1, femdl);
            fwrite(&pos_fp, sizeof(PACK_VEC3_NRM_S16), 1, femdl);

            if(emdl_flags.use_normals)
            {
                fread(&nrm[0], 4, 3, fmdl);
                PACK_VEC3_NRM_S16 nrmp = f32_to_nrm_vec16(nrm[0], nrm[1], nrm[2]);
                fwrite(&nrmp, sizeof(PACK_VEC3_NRM_S16), 1, femdl);
            }
        }
