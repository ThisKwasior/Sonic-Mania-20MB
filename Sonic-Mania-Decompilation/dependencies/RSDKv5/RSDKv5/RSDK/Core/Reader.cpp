...
#include "RSDK/Tenmb/rsdk_zlib.h"
...
process_data_rsdk:
    ...
    else if(1)
    {
        ...
        PrintLog(PRINT_NORMAL, "READER Data Embedded Size - %u", DATA_ZLIB_SIZE);
        PrintLog(PRINT_NORMAL, "READER Data Real Size     - %u", DATA_REAL_SIZE);
        
        uint8_t* decompressed = (uint8_t*)malloc(DATA_REAL_SIZE);
        
        mz_ulong dest_len = DATA_REAL_SIZE;
        mz_ulong source_len = DATA_ZLIB_SIZE;
        int stat = mz_uncompress2(&decompressed[0], &dest_len, (unsigned char*)&DATA_START[0], &source_len);
        
        PrintLog(PRINT_NORMAL, "READER mz_uncompress2     - %d | %d", stat, MZ_OK);
        PrintLog(PRINT_NORMAL, "READER mz_uncompress2     - %u | %u", dest_len, source_len);
        ...
        CloseFile(&info);
        InitFileInfo(&info);
        info.fileSize = DATA_REAL_SIZE;
        info.readPos = 0;
        info.usingFileBuffer = true;
        //info.fileBuffer = (uint8_t*)malloc(DATA_REAL_SIZE);
        info.file = (FileIO*)&decompressed[0];
        info.fileBuffer = &decompressed[0];
        
        goto process_data_rsdk;
    }
#endif
    else {
        useDataPack = false;
