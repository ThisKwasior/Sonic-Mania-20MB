...
#include "RSDK/Tenmb/stb_image.h"
...
bool32 ImageGIF::Load(const char *fileName, bool32 loadHeader)
{
    ...
    if (fileName) {
        ...
        PrintLog(PRINT_NORMAL, "%s | Size %u %u", fileName, width, height);

        if (loadHeader)
            return true;
    }

    int16 pal_count = ReadInt16(&info);
    PrintLog(PRINT_NORMAL, "%s | Pal count %u", fileName, pal_count);
    ...
    if (palette && pixels) {
        ...
        int32 c = 0;
        do {
            uint16 clr565 = ReadInt16(&info);
            RGB888 clr888 = rgb565_to_888(*(RGB565*)&clr565);
            palette[c] = (clr888.r << 16) | (clr888.g << 8) | (clr888.b << 0);
            //palette[c] = (((uint8_t)(c)) << 16) | (((uint8_t)(c)) << 8) | (((uint8_t)(c)) << 0);
            ++c;
        } while (c != pal_count);

        PrintLog(PRINT_NORMAL, "%s | File size %u", fileName, info.fileSize);

        uint32 png_size = info.fileSize - (10 + pal_count*2);
        PrintLog(PRINT_NORMAL, "%s | PNG size %u", fileName, png_size);
        
        Seek_Set(&info, 10 + pal_count*2);
        
        stbi_uc* png_data = (stbi_uc*)malloc(png_size);
        PrintLog(PRINT_NORMAL, "%s | PNG data ptr %p", fileName, png_data);
        ReadBytes(&info, png_data, png_size);
        int x, y, n;
        unsigned char* png_pixels = stbi_load_from_memory(png_data, png_size, &x, &y, &n, 1);
        PrintLog(PRINT_NORMAL, "%s | PNG stbi %d %d %d", fileName, x, y, n);
        
        memcpy(pixels, png_pixels, width * height);
        free(png_pixels);
        free(png_data);
