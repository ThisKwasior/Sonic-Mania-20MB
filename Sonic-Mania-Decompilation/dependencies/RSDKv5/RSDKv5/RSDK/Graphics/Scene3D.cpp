...
#include <RSDK/Tenmb/vrle.h>
#include <RSDK/Tenmb/rgb.h>
#include <RSDK/Tenmb/bit_pack.h>
...
uint16 RSDK::LoadMesh(const char *filename, uint8 scope)
{
    ...
    if (LoadFile(&info, fullFilePath, FMODE_RB)) {
        ...
        if(model->flags & MODEL_USECOLOURS)
        {
            ...
            while(v < model->vertCount)
            {
                int8_t vrle_head_s8 = ReadInt8(&info);
                int16_t vrle_data_s16 = ReadInt16(&info);
                VRLE_HEADER vrle_head = *(VRLE_HEADER*)&vrle_head_s8;
                RGBA5551 vrle_col = *(RGBA5551*)&vrle_data_s16;
                RGBA8888 vrle_col_8888 = rgba_unpack_5551(vrle_col);
                
                for(int32 v2 = 0; v2 != vrle_head.len; ++v2)
                {
                    model->colors[v+v2].color = vrle_col_8888.a << 24
                                                | vrle_col_8888.r << 16
                                                | vrle_col_8888.g << 8
                                                | vrle_col_8888.b;
                }
        ...
        for(int32 i = 0; i < model->indexCount; ++i)
        {
            model->indices[i] = ReadInt16(&info);
        }

        float scale_facs[3] = {0};
        scale_facs[0] = ReadSingle(&info);
        scale_facs[1] = ReadSingle(&info);
        scale_facs[2] = ReadSingle(&info);

        float verts_temp[3] = {0};
        //PACK_VEC3_NRM_S24 vert_packnrm_temp = {0};
        PACK_VEC3_NRM_S16 vert_packnrm_temp = {0};
        PACK_VEC3_NRM_S16 normal_packed_temp = {0};
        ...
        for(int32 f = 0; f < model->frameCount; ++f)
        {
            for(int32 v = 0; v < model->vertCount; ++v)
            {
                ...
                //vert_packnrm_temp.x = ReadInt8(&info);
                //vert_packnrm_temp.y = ReadInt8(&info);
                //vert_packnrm_temp.z = ReadInt8(&info);

                int16_t tmpver = ReadInt16(&info);
                vert_packnrm_temp = *(PACK_VEC3_NRM_S16*)&tmpver;

                //packed_nrm_vec24_to_f32(vert_packnrm_temp, &verts_temp[0], &verts_temp[1], &verts_temp[2]);
                packed_nrm_vec16_to_f32(vert_packnrm_temp, &verts_temp[0], &verts_temp[1], &verts_temp[2]);

                model->vertices[(f * model->vertCount) + v].x = (int32)(verts_temp[0]*scale_facs[0] * 0x100);
                model->vertices[(f * model->vertCount) + v].y = (int32)(verts_temp[1]*scale_facs[1] * 0x100);
                model->vertices[(f * model->vertCount) + v].z = (int32)(verts_temp[2]*scale_facs[2] * 0x100);
                ...
                {
                    int16_t tmpnrm = ReadInt16(&info);
                    normal_packed_temp = *(PACK_VEC3_NRM_S16*)&tmpnrm;
                    packed_nrm_vec16_to_f32(normal_packed_temp, &verts_temp[0], &verts_temp[1], &verts_temp[2]);
                    
                    model->vertices[(f * model->vertCount) + v].nx = (int32)(verts_temp[0] * 0x10000);
                    model->vertices[(f * model->vertCount) + v].ny = (int32)(verts_temp[1] * 0x10000);
                    model->vertices[(f * model->vertCount) + v].nz = (int32)(verts_temp[2] * 0x10000);
                }
            }
        }
